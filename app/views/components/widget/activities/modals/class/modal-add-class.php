<?php
/**
 * Modal Add Class Component
 * Modal for adding new class
 */
?>

<!-- Modal Backdrop -->
<div id="modal-add-class" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <!-- Modal Container -->
    <div class="bg-white-neutral border border-border-soft rounded-[16px] w-[621px] px-[24px] py-[20px]">
        ...
        <div class="flex items-start justify-between mb-[32px]">
            <h3 class="font-bold text-[20px] leading-[100%] text-black-soft">Tambah Kelas Baru</h3>
            <button type="button" id="close-modal-add-class" class="text-black-highlight hover:text-black-soft transition-colors">
        ...
        <div class="flex items-start gap-[20px] mb-[24px] justify-center">
            ...
            <div class="flex flex-col items-center gap-[8px]">
                <div class="w-[124px] h-[124px] rounded-[14px] bg-gray-placeholder flex items-center justify-center flex-shrink-0">
                    <img id="preview-image" src="" alt="Preview" class="w-full h-full object-cover rounded-[14px] hidden">
                    <svg class="w-12 h-12 text-white-shadow" id="placeholder-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                </div>
                <p id="caution-text-add-class" class="text-[10px] leading-[16px] text-white-shadow font-normal">Maksimal ukuran file 2MB</p>
            </div>
            
            <!-- Upload and Delete Buttons -->
            <div class="flex flex-col justify-between h-[124px]">
                <!-- Upload Button -->
                <label for="class-image-input" class="cursor-pointer">
                ...
                <input type="file" id="class-image-input" accept="image/*" class="hidden">
        ...
        <!-- Form Fields -->
        <form id="form-add-class" class="space-y-[24px]">
            <!-- Nama Kelas -->
            <div>
                <label for="class-name" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                    Nama Kelas
                </label>
                <input 
                    type="text" 
            ...
            <div>
                <label for="age-range" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                    Rentang Usia Siswa di Kelas
                </label>
                <input 
                    type="text" 
            ...
            <div class="grid grid-cols-2 gap-[20px]">
                ...
                <div>
                    <label for="duration" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                        Durasi Kelas per Hari
                    </label>
                    <input 
                        type="text" 
                ...
                <div>
                    <label for="max-students" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                        Maksimal Siswa di Dalam Kelas
                    </label>
                    <input 
                        type="text" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const openBtn = document.getElementById('btn-add-class');
    const closeBtn = document.getElementById('close-modal-add-class');
    const imageInput = document.getElementById('class-image-input');
    ...
    if (imageInput) {
        imageInput.addEventListener('change', function(e) {
            ...
            const cautionText = document.getElementById('caution-text-add-class');
                ...
                reader.onload = function(e) {
                    ...
                    if (deleteButtonWrapper) {
                        deleteButtonWrapper.innerHTML = `
                            <button type="button" id="delete-image-btn" class="btn-component inline-flex items-center justify-center font-bold transition-all duration-200 py-3 px-6 rounded-xl bg-white-neutral text-primary font-normal text-base leading-[28px] border border-border-light hover:bg-white-secondary cursor-pointer">
                                Hapus Gambar
                            </button>
                        `;
                        
                        // Re-attach event listener
                        const newDeleteBtn = document.getElementById('delete-image-btn');
                        if (newDeleteBtn) {
    ...
    function validateForm() {
        ...
        if (allFilled) {
            ...
            submitButtonWrapper.innerHTML = `
                <button type="submit" id="submit-add-class-btn" class="btn-component inline-flex items-center justify-center font-bold transition-all duration-200 py-[12px] px-6 rounded-xl bg-primary text-white font-bold text-base h-[52px] hover:opacity-90 active:scale-95 cursor-pointer">
                    Tambah Kelas Baru
                </button>
            `;
        } else {
            ...
            submitButtonWrapper.innerHTML = `
                <button type="submit" id="submit-add-class-btn" disabled class="btn-component inline-flex items-center justify-center font-bold transition-all duration-200 py-[12px] px-[24px] rounded-xl bg-white-secondary text-white-shadow font-normal text-base leading-[28px] border border-border-light h-[52px] opacity-50 cursor-not-allowed">
                    Tambah Kelas Baru
                </button>
            `;
        }
    ...
    const formInputs = ['class-name', 'age-range', 'duration', 'max-students'];
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            formData.append('age_range', document.getElementById('age-range').value);
            ...
            // Send AJAX request
            fetch('/admin/activities/classes/store', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.success) {
                    modal.classList.add('hidden');
                    resetForm();
                    showToast('Kelas berhasil ditambahkan!', 'success', 3000);
                    setTimeout(() => {
