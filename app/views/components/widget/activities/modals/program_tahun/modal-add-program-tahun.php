<?php
/**
 * Modal Add Program Tahun Ajaran Component
 * Modal for adding new program tahun ajaran
 */
?>

<!-- Modal Backdrop -->
<div id="modal-add-program-tahun" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <!-- Modal Container -->
    <div class="bg-white-neutral border border-border-soft rounded-[16px] w-[621px] px-[24px] py-[20px]">
        ...
        <div class="flex items-start justify-between mb-[32px]">
            <h3 class="font-bold text-[20px] leading-[100%] text-black-soft">Tambah Program Tahun Ajaran Baru</h3>
            <button type="button" id="close-modal-add-program-tahun" class="text-black-highlight hover:text-black-soft transition-colors">
        ...
        <div class="flex items-start gap-[20px] mb-[24px] justify-center">
            ...
            <div class="flex flex-col items-center gap-[8px]">
                <div class="w-[124px] h-[124px] rounded-[14px] bg-gray-placeholder flex items-center justify-center flex-shrink-0">
                    <img id="preview-image-program-tahun" src="" alt="Preview" class="w-full h-full object-cover rounded-[14px] hidden">
                    <svg class="w-12 h-12 text-white-shadow" id="placeholder-icon-program-tahun" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                </div>
                <p id="caution-text-program-tahun" class="text-[10px] leading-[16px] text-white-shadow font-normal">Maksimal ukuran file 2MB</p>
            </div>
            
            <!-- Upload and Delete Buttons -->
            <div class="flex flex-col justify-between h-[124px]">
                <!-- Upload Button -->
                <label for="program-tahun-image-input" class="cursor-pointer">
                ...
                <input type="file" id="program-tahun-image-input" accept="image/*" class="hidden">
                
                <!-- Delete Button -->
                <div id="delete-button-wrapper-program-tahun">
                    <?php component('button', [
                        'text' => 'Hapus Gambar',
                        'variant' => '10',
                        'type' => 'button',
                        'id' => 'delete-image-btn-program-tahun'
                    ]); ?>
                </div>
            </div>
        </div>
        ...
        <!-- Form Fields -->
        <form id="form-add-program-tahun" class="space-y-[24px]">
            <!-- Nama Program -->
            <div>
                <label for="program-name" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                    Nama Program
                </label>
                <input 
                    type="text" 
            ...
            <!-- Deskripsi Program -->
            <div>
                <label for="program-description" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                    Deskripsi Program
                </label>
                <textarea 
            ...
            <div class="pt-[8px] flex justify-center">
                <div id="submit-button-wrapper-program-tahun">
                    <?php component('button', [
                        'text' => 'Tambah Program Baru',
                        'variant' => '10',
                        'type' => 'submit',
                        'id' => 'submit-add-program-tahun-btn'
                    ]); ?>
                </div>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('modal-add-program-tahun');
    const openBtn = document.getElementById('btn-add-program-tahun');
    const closeBtn = document.getElementById('close-modal-add-program-tahun');
    const imageInput = document.getElementById('program-tahun-image-input');
    const previewImage = document.getElementById('preview-image-program-tahun');
    const placeholderIcon = document.getElementById('placeholder-icon-program-tahun');
    const deleteImageBtn = document.getElementById('delete-image-btn-program-tahun');
    ...
    if (imageInput) {
        imageInput.addEventListener('change', function(e) {
            ...
            const cautionText = document.getElementById('caution-text-program-tahun');
    ...
    function validateForm() {
        ...
        const programDescription = document.getElementById('program-description').value.trim();
        ...
        if (allFilled) {
            ...
            submitButtonWrapper.innerHTML = `
                <button type="submit" id="submit-add-program-tahun-btn" disabled class="btn-component inline-flex items-center justify-center font-bold transition-all duration-200 py-[12px] px-[24px] rounded-xl bg-white-secondary text-white-shadow font-normal text-base leading-[28px] border border-border-light h-[52px] opacity-50 cursor-not-allowed">
                    Tambah Program Baru
                </button>
            `;
        }
    ...
    const formInputs = ['program-name', 'program-description'];
    ...
    // Form submit
    const form = document.getElementById('form-add-program-tahun');
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // Send AJAX request
            fetch('/admin/activities/programs-tahun/store', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.success) {
                    modal.classList.add('hidden');
                    resetForm();
                    showToast('Program berhasil ditambahkan!', 'success', 3000);
                    setTimeout(() => {
