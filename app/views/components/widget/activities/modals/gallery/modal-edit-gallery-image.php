...
<!-- Modal Backdrop -->
<div id="modal-edit-gallery-image" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <!-- Modal Container -->
    <div class="bg-white-neutral border border-border-soft rounded-[16px] w-[621px] px-[24px] py-[20px]">
        ...
        <div class="flex items-start justify-between mb-[32px]">
            ...
            <button type="button" id="close-modal-edit-gallery-image" class="text-black-highlight hover:text-black-soft transition-colors">
        ...
        <div class="flex items-start gap-[20px] mb-[24px] justify-center">
            ...
            <div class="flex flex-col items-center gap-[8px]">
                <div class="w-[124px] h-[124px] rounded-[14px] bg-gray-placeholder flex items-center justify-center flex-shrink-0">
                    <img id="preview-image-gallery-edit" src="" alt="Preview" class="w-full h-full object-cover rounded-[14px] hidden">
                    <svg class="w-12 h-12 text-white-shadow" id="placeholder-icon-gallery-edit" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                </div>
                <p id="caution-text-gallery-edit" class="text-[10px] leading-[16px] text-white-shadow font-normal">Maksimal ukuran file 2MB</p>
            </div>
            
            <!-- Upload Button -->
            <div class="flex flex-col justify-between h-[124px]">
                <label for="gallery-image-input-edit" class="cursor-pointer">
                ...
                <input type="file" id="gallery-image-input-edit" accept="image/*" class="hidden">
            </div>
        </div>
        
        <!-- Form Fields -->
        <form id="form-edit-gallery-image" class="space-y-[24px]">
            <!-- Hidden fields -->
            <input type="hidden" id="gallery-image-id-edit" name="gallery_image_id">
            <input type="hidden" id="gallery-program-id-edit" name="program_id">
            
            <!-- Deskripsi Foto -->
            <div>
                <label for="photo-description-edit" class="block font-normal text-[12px] leading-[21px] text-black-highlight mb-[8px]">
                    Deskripsi Foto
                </label>
                <textarea 
                    id="photo-description-edit" 
            ...
            <div class="flex items-center" style="margin-top: 12px;">
                <input 
                    type="checkbox" 
                    id="set-as-cover-edit" 
                    name="set_as_cover"
                    class="w-[12px] h-[12px] border border-border-light rounded cursor-pointer"
                    style="accent-color: <?= colors('primary') ?>;"
                >
                <label for="set-as-cover-edit" class="ml-[8px] cursor-pointer" style="font-weight: 400; font-size: 12px; line-height: 20px; color: <?= colors('white_shadow') ?>;">
            ...
            <div class="pt-[8px] flex items-center justify-center gap-[12px]" style="margin-top: 32px;">
                ...
                <!-- Submit Button -->
                <div id="submit-button-wrapper-gallery-edit">
                    <?php component('button', [
...
window.openEditGalleryImageModal = function(galleryData) {
    const modal = document.getElementById('modal-edit-gallery-image');
    const previewImage = document.getElementById('preview-image-gallery-edit');
    const placeholderIcon = document.getElementById('placeholder-icon-gallery-edit');
    
    // Store is_program_image flag
    modal.dataset.isProgramImage = galleryData.is_program_image ? 'true' : 'false';
    
    // Fill form fields
    document.getElementById('gallery-image-id-edit').value = galleryData.id;
    document.getElementById('gallery-program-id-edit').value = galleryData.program_id;
    document.getElementById('photo-description-edit').value = galleryData.description;
    document.getElementById('set-as-cover-edit').checked = galleryData.is_cover == 1;
    ...
    const deletePhotoBtn = document.getElementById('delete-gallery-photo-btn');
    if (deletePhotoBtn) {
...
document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('modal-edit-gallery-image');
    const closeBtn = document.getElementById('close-modal-edit-gallery-image');
    const imageInput = document.getElementById('gallery-image-input-edit');
    const previewImage = document.getElementById('preview-image-gallery-edit');
    const placeholderIcon = document.getElementById('placeholder-icon-gallery-edit');
    const deletePhotoBtn = document.getElementById('delete-gallery-photo-btn');
    ...
    if (imageInput) {
        imageInput.addEventListener('change', function(e) {
            ...
            const cautionText = document.getElementById('caution-text-gallery-edit');
    ...
    function validateEditGalleryForm() {
        const photoDescription = document.getElementById('photo-description-edit').value.trim();
        
        const allFilled = photoDescription;
        const submitButtonWrapper = document.getElementById('submit-button-wrapper-gallery-edit');
        
        if (allFilled) {
            // Change to variant 1 (primary)
            submitButtonWrapper.innerHTML = `
                <button type="submit" id="submit-edit-gallery-btn" class="btn-component inline-flex items-center justify-center font-bold transition-all duration-200 py-[12px] px-6 rounded-xl bg-primary text-white font-bold text-base h-[52px] hover:opacity-90 active:scale-95 cursor-pointer">
                    Ubah Informasi Foto
                </button>
            `;
        } else {
    ...
    // Make validateEditGalleryForm available globally
    window.validateEditGalleryForm = validateEditGalleryForm;
    
    // Add input listener for form validation
    const descInput = document.getElementById('photo-description-edit');
    if (descInput) {
        descInput.addEventListener('input', validateEditGalleryForm);
    }
    
    // Form submit (Update)
    const form = document.getElementById('form-edit-gallery-image');
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            formData.append('description', document.getElementById('photo-description-edit').value);
            formData.append('is_cover', document.getElementById('set-as-cover-edit').checked ? '1' : '0');
            ...
            if (isProgramImage) {
                ...
                endpoint = `/admin/activities/programs-tahun/${programId}/update-program-image`;
            } else {
                // Update gallery image
                endpoint = `/admin/activities/programs-tahun/${programId}/gallery/${galleryImageId}/update`;
            }
            ...
            if (imageInput.files[0]) {
                ...
                if (isProgramImage) {
                    ...
                    if (confirm('Apakah Anda yakin ingin menghapus gambar program ini?')) {
                        ...
                        fetch(`/admin/activities/programs-tahun/${programId}/delete-program-image`, {
                            method: 'POST'
                        })
                        .then(response => response.json())
                        .then(data => {
